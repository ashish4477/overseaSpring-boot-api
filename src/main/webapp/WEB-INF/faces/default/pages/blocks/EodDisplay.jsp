...
<div class="body-content">
  <div id="eod-display" class="<c:if test="${not (isSvid and isEod)}">svid </c:if>page-form">
    <div class="hd">
      <div class="hd-inner">
        <h1 class="title">
        ...
        <div id="electiondeadlines" class="accordian rounded-5">
          ...
          <div class="pnl">
            <c:forEach items="${localElections}" var="election" varStatus="inx">
              <table class="accordian-content rounded-5 election-deadlines" cellspacing="0">
                ...
                <tr class="r1">
                  ...
                  <td  class="b-left"><c:choose><c:when test="${not empty election.citizenRegistration}">${election.citizenRegistration}</c:when><c:otherwise>To Be Announced</c:otherwise></c:choose>
                  </td>
                  <td class="b-left"><c:choose><c:when test="${not empty election.militaryRegistration}">${election.militaryRegistration}</c:when><c:otherwise>To Be Announced</c:otherwise></c:choose></td>
                </tr>
                <tr class="r2">
                  <td class="first-col">Ballot Request for Registered Voter</td>
                  <td class="b-left"><c:choose><c:when test="${not empty election.citizenBallotRequest}">${election.citizenBallotRequest}</c:when><c:otherwise>To Be Announced</c:otherwise></c:choose></td>
                  <td class="b-left"><c:choose><c:when test="${not empty election.militaryBallotRequest}">${election.militaryBallotRequest}</c:when><c:otherwise>To Be Announced</c:otherwise></c:choose></td>
                </tr>
                <tr class="r3">
                  <td class="first-col">Ballot Return</td>
                  <td class="b-left"><c:choose><c:when test="${not empty election.citizenBallotReturn}">${election.citizenBallotReturn}</c:when><c:otherwise>To Be Announced</c:otherwise></c:choose></td>
                  <td class="b-left"><c:choose><c:when test="${not empty election.militaryBallotReturn}">${election.militaryBallotReturn}</c:when><c:otherwise>To Be Announced</c:otherwise></c:choose></td>
                </tr>
                <tr class="r4">
            ...
            <c:if test="${empty localElections}">
              <p class="accordian-content rounded-5 election-deadlines" style="padding:5px;">There are no elections on record for this location. Election dates and deadlines will be posted when available. Questions or comments can be sent to <a href="mailto:info@overseasvotefoundation.org">info@overseasvotefoundation.org</a></p>
            </c:if>
          </div>
        </div>
                    ...
                    <c:forEach items="${voterTypeTransmissionMethods.groupedItems}" var="items" varStatus="indx">
                      ...
                      <tr class="${style}">
                        ...
                        <td><c:choose>
                          ...
                          <span class="icon-ok" aria-hidden="true"></span>                           </c:when>
