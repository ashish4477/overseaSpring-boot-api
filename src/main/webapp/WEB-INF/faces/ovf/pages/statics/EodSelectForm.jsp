      ...
      var params = { 'stateInputId':'select_state', 'regionInputId':'votingRegion', 'url':'${getJsonRegionUrl}' };
		...
		<form action="<c:url value="${pageUrl}"/>" role="form" name="eodForm" class="bc-form" method="get" id="eodForm">
            ...
            <div class="form-group">
              ...
              <label for="select_state" class="oneline one-line select select-state-field">
                <span>Choose a State:</span>
              </label>
              <c:choose>
                <c:when test="${useCleanUrl}">
                  <select class="field form-control" name="stateName" id="select_state" required="required" >  <%--onchange="getJsonRegions( params )"--%>
                ...
                <c:otherwise>
                  <select class="field form-control" name="stateId" id="select_state" required="required" > <%-- onchange="getJsonRegions( params )"--%>
                    <option value="0">- Select Your State -</option>
                    <c:forEach items="${states}" var="state">
				...
				<c:if test="${isEod}">
					...
					<select id="votingRegion" class="field form-control" name="regionId" required="required">
