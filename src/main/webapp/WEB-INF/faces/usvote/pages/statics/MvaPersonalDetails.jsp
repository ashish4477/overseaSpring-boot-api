      ...
      <div class="col-xs-12 col-md-6">
        ...
        <fieldset class="loginInfo">
            ...
            <div class="usernameConfirmation form-group">
                <label for="usernameConfirmation" path="usernameConfirmation" class="usernameConfirmation control-label"> Please Confirm Your Email *</label>
                <input path="usernameConfirmation" name="usernameConfirmation" id="usernameConfirmation" class="form-control usernameConfirmation"/>
                <span id="confirmMessage" class="errorMsg"></span>
            </div>
            </c:if>
        ...
        <fieldset class="birthInfo">
          <div class="row">
            <div class="birthMonth form-group col-4">
                ...
                <form:select  id="birthMonth" path="birthMonth" cssErrorClass="error form-control" class="form-control">
            ...
            <div class="birthDate form-group col-4">
                ...
                <form:select id="birthDate" path="birthDate" cssErrorClass="error form-control" class="form-control">
                    <form:option value="0" label="Select"/>
            ...
            <div class="birthYear form-group col-4">
                ...
                <form:select id="birthYear" path="birthYear" cssErrorClass="error form-control" class="form-control">
                <c:set var="yearNumb" value="${yearNumber-16}" scope="page"/>
                    <form:option value="0" label="Select"/>
                    <c:forEach begin="1900" end="${yearNumb}" var="i" step="1">
                      <c:set var="j" value="${1900+yearNumb-i}"/>
...
$(function() {
    ...
    $("#userForm").submit(function(e) {
        if ($('#username').length > 0 && $('#usernameConfirmation').length > 0) {
            validateUsernameConfirmation();
        }
        if ($('#confirmMessage').length > 0 && $('#confirmMessage').text().trim().length > 0) {
            e.preventDefault();
        }
    });

    if ($('#username\\.errors').length > 0 && $('#usernameConfirmation').val().trim().length == 0) {
        $('.usernameConfirmation').addClass( "error" );
        $('#confirmMessage').html("An email address confirmation is required.");
    }

    const validateUsernameConfirmation = function() {
        if ($('#username').val().trim().length > 0 && $('#usernameConfirmation').val().trim().length >= 0) {
            if($('#username').val() != $('#usernameConfirmation').val()) {
                $('.usernameConfirmation').addClass( "error" );
                $('#confirmMessage').html("Your Emails do not match, please correct your entry");
            }
            else{
                $('.usernameConfirmation').removeClass( "error" );
