...
<c:if test="${empty param.cStateId or isEod or param.showForm}">
	<c:if test="${isEod}">
		...
		<form action="<c:url value="${pageUrl}"/>" role="form" name="eodForm" class="bc-form election-dates-form" method="get" id="eodForm">
			<c:choose>
				<c:when test="${isEod}">
					<h4 id="eod-form-title">Election Official Directory</h4>
				</c:when>
				<c:otherwise>
					<h4 id="eod-form-title">Quick Take on Voting in My State</h4>
				</c:otherwise>
			</c:choose>
		...
		<div class="form-blurb-block">
			<p>Voter registration ID, ways to vote in your state, whether you can register to vote online - all of these election details are state-specific. US Vote keeps track of them for you.</p>
			<p>If you have questions like, can I vote? What ID should I bring to vote? Can I send in my forms by email? Do I need an excuse to request an absentee ballot? Where do I vote?…. you are in the right place!</p>
			<p>This State Voter Information guide answers all these voter questions and more. Nonpartisan, reliable, and easy to access. Everything you need to know to vote in your state.</p>
		</div>
