...
<c:if test="${updated}">
  <div id="update-message" class="col-12 alert alert-success">Your account information has been updated</div>
  </c:if>
  ...
  <div class="row">
      <div class="col-12 col-sm-4 offset-sm-1 voterAccount">
    ...
    <div class="row answers">
      ...
      <ul class="list-unstyled">
       ...
       <c:forEach items="${elections}" var="myElections">
          <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/>> <c:out value="${myElections}" /></li>
       </c:forEach>
       <c:if test="${not empty extendedProfile.voterParticipationOther}">
         <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> ${extendedProfile.voterParticipationOther}</li>
      </c:if>
      </ul>
      ...
      <ul class="list-unstyled">
       ...
       <c:forEach items="${outreach}" var="myOutreach">
         <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> <c:out value="${myOutreach}" /></li>
       </c:forEach>
        <c:if test="${not empty extendedProfile.outreachParticipationOther}">
          <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> ${extendedProfile.outreachParticipationOther}</li>
        </c:if>
      </ul>
      </div>
    </c:if>
      <c:if test="${not empty extendedProfile.volunteeringArr}">
            ...
            <ul class="list-unstyled">
             ...
             <c:forEach items="${volunteering}" var="myVolunteering">
                <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> <c:out value="${myVolunteering}" /></li>
             </c:forEach>
             <c:if test="${not empty extendedProfile.volunteeringOther}">
               <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> ${extendedProfile.volunteeringOther}</li>
            </c:if>
            </ul>
            </div>
            </c:if>
     ...
     <c:if test="${not empty extendedProfile.socialMediaArr}">
      ...
      <ul class="list-unstyled">
       ...
       <c:forEach items="${socialMedia}" var="myMediaTypes">
         <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> <c:out value="${myMediaTypes}" /></li>
       </c:forEach>
        <c:if test="${not empty extendedProfile.socialMediaOther}">
          <li><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/> ${extendedProfile.socialMediaOther}</li>
        </c:if>
      </ul>
      </div>
    </c:if>
