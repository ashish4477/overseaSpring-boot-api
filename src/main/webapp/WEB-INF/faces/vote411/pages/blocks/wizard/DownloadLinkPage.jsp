...
<div class="page-form col-12 col-sm-10 offset-sm-1">
    <c:choose>
    ...
    <div class="bd-inner">
                        ...
                        <script>
                         jQuery(function() {
                          ...
                          document.location = url;
                            ...
                            <div id="update-message" class="alert alert-warning">The download link has expired. Please <a href="<c:url value="/voter-registration-absentee-voting.htm"/>">click here</a> to restart the data entry process.</div>
                        </c:when>
                        <c:otherwise>
                            <div id="update-message" class="alert alert-warning">There is no form to download. Please <a href="<c:url value="/voter-registration-absentee-voting.htm"/>">click here</a> to restart the data entry process.</div>
                        </c:otherwise>
                    </c:choose>
                </div>
            </c:if>
        ...
        <c:set var="formType" value="Form"/>
          ...
          <div class="download-box complete" style="display:none;">
                <div class="download-box">
                  <div id="update-message" class="alert alert-success"><img class="icon-check" src="<c:url value="/img/icons/check.svg"/>"/>&nbsp; Your Prepared ${formType} is Available for Download.</div>
          <h3>To complete your request you must take the following steps:</h3>
           <br/>
          <ol>
            <li> Download Your Completed ${formType}</li>
            <li> Verify to Check</li>
            ...
            <li> Send the Signed, Original Printed Document by Mail in Order to Assure You Meet the Signature Requirements <span class="hint">(even if you fax or email your document)</span></li>
              ...
              <span id="pdf-download-link"><a href="<c:url value="/CreatePdf.htm?flow=${wizardContext.flowType}&generationUUID=${wizardContext.wizardResults.uuid}"/>" target="_new" id="download-link" class="button">Download Your Form</a></span>
            </div>
